...
- Nothing readable leaves the Pi. This satisfies the sensitivity of
  `GLOBAL_MEMORY.md` and the hard constraint never to connect Ian's pseudonymous
  Seattle identity to his real identity in any published/exposed form.
...
- No credentials are stored in any repo or synced file. `GLOBAL_MEMORY.md`
  content is never published anywhere public; the pseudonymous Seattle identity
  is never linked to Ian's real identity in any exposed artifact.
